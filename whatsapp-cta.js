(() => {
  ...
  notice.textContent = 'Need a quick response? Chat with us on WhatsApp.';
  ...
  style.textContent = `
    #${id} {
      ...
      text-decoration: none; transition: background .3s ease, box-shadow .3s ease, transform .3s ease;
      width: 52px; z-index: 1001;
    }
    #${id}:hover, #${id}:focus-visible { background: #1a9e9e; color: #fff; transform: translateY(-2px); }
    #${id}.is-whatsapp-green { background: #25D366; box-shadow: 0 8px 24px rgba(37, 211, 102, .48); }
    ...
    #${id}-notice {
      background: #fff; border-radius: 10px; bottom: 88px; box-shadow: 0 8px 24px rgba(17, 43, 66, .18);
      color: #173f5f; font: 600 12px/1.35 Poppins, Arial, sans-serif; left: 24px; max-width: 210px;
      padding: 10px 12px; pointer-events: none; position: fixed; z-index: 1000;
    }
    #${id}-notice::after { border: 8px solid transparent; border-top-color: #fff; bottom: -16px; content: ''; left: 18px; position: absolute; }
    @media (max-width: 640px) {
      #${id} { bottom: 16px; height: 48px; left: 16px; width: 48px; }
      #${id}-notice { bottom: 76px; font-size: 11px; left: 16px; max-width: 190px; }
    }
  `;
  ...
  const highlight = () => {
    link.classList.add('is-whatsapp-green');
    window.setTimeout(() => link.classList.remove('is-whatsapp-green'), 2200);
  };

  window.setTimeout(highlight, 2800);
  window.setInterval(highlight, 12000);
})();
